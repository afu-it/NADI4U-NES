...
class ReminderStore {
  constructor() {
    ...
    this.quarterlyDoneKey = 'nadi_quarterly_reminder_done';
    this.data = this.loadLocal();
    this.quarterlyDone = this.loadQuarterlyDone();
  }
  ...
  loadQuarterlyDone() {
    try {
      const stored = window.safeStorage.getItem(this.quarterlyDoneKey);
      if (stored) {
      ...
      console.error('Error loading quarterly reminder completion state:', error);
    }
    return {};
  }
  ...
  saveQuarterlyDone() {
    try {
      window.safeStorage.setItem(this.quarterlyDoneKey, JSON.stringify(this.quarterlyDone));
    } catch (error) {
      console.error('Error saving quarterly reminder completion state:', error);
    }
  }
  ...
  async remove(type, id) {
    ...
    if (!client) {
      ...
      if (type === 'quarterly') {
        this.clearQuarterlyDoneState(id);
      }
      this.saveLocal();
      return true;
    }
  ...
  getQuarterlyCycleStorageKey(id, cycleKey) {
    return `${id}:${cycleKey}`;
  }

  isQuarterlyCycleDone(id, cycleKey) {
    return Boolean(this.quarterlyDone[this.getQuarterlyCycleStorageKey(id, cycleKey)]);
  }

  setQuarterlyCycleDone(id, cycleKey, isDone) {
    const storageKey = this.getQuarterlyCycleStorageKey(id, cycleKey);
    if (isDone) {
      this.quarterlyDone[storageKey] = true;
    } else {
      delete this.quarterlyDone[storageKey];
    }
    this.saveQuarterlyDone();
  }

  clearQuarterlyDoneState(id) {
    Object.keys(this.quarterlyDone).forEach((key) => {
      if (key.startsWith(`${id}:`)) {
        delete this.quarterlyDone[key];
      }
    });
    this.saveQuarterlyDone();
  }
...
class DateUtils {
  ...
  static getQuarterlyCycleInfo(quarterMonth) {
    const now = new Date();
    now.setHours(0, 0, 0, 0);

    const month = parseInt(quarterMonth, 10) - 1;
    const currentYearDueDate = new Date(now.getFullYear(), month + 1, 0);
    currentYearDueDate.setHours(0, 0, 0, 0);

    const graceEndDate = new Date(currentYearDueDate);
    graceEndDate.setMonth(graceEndDate.getMonth() + 1, 7);
    graceEndDate.setHours(0, 0, 0, 0);

    if (now <= currentYearDueDate) {
      return {
        dueDate: currentYearDueDate,
        nextDate: currentYearDueDate,
        graceEndDate,
        isActiveWindow: false,
        isGraceWindow: false,
        isPastGraceWindow: false,
        cycleKey: DateUtils.toIsoDate(currentYearDueDate)
      };
    }
    ...
    const nextYearDueDate = new Date(now.getFullYear() + 1, month + 1, 0);
    nextYearDueDate.setHours(0, 0, 0, 0);
    const nextYearGraceEnd = new Date(nextYearDueDate);
    nextYearGraceEnd.setMonth(nextYearGraceEnd.getMonth() + 1, 7);
    nextYearGraceEnd.setHours(0, 0, 0, 0);

    return {
      dueDate: nextYearDueDate,
      nextDate: nextYearDueDate,
      graceEndDate: nextYearGraceEnd,
      isActiveWindow: false,
      isGraceWindow: false,
      isPastGraceWindow: true,
      cycleKey: DateUtils.toIsoDate(nextYearDueDate)
    };
  }

  static getActiveQuarterlyTarget() {
    const quarterTargets = DateUtils.getQuarterMonths().map((month) => ({
      month,
      cycleInfo: DateUtils.getQuarterlyCycleInfo(month)
    }));

    const graceTarget = quarterTargets.find((target) => target.cycleInfo.isGraceWindow);
    if (graceTarget) return graceTarget;

    quarterTargets.sort((a, b) => a.cycleInfo.nextDate.getTime() - b.cycleInfo.nextDate.getTime());
    return quarterTargets[0] || null;
  }
...
class ReminderUI {
  ...
  getQuarterlyReminders() {
    const activeTarget = DateUtils.getActiveQuarterlyTarget();
    if (!activeTarget) return [];

    return [{
      id: 'quarterly-auto-mcmc-report',
      type: 'quarterly',
      title: `MCMC REPORT - ${DateUtils.getQuarterName(activeTarget.month)}`,
      description: '',
  ...
  getAllRemindersForUi() {
    const weekly = this.store.getAll('weekly').map((reminder) => ({ ...reminder, type: 'weekly' }));
    const monthly = this.store.getAll('monthly').map((reminder) => ({ ...reminder, type: 'monthly' }));
    const userQuarterly = this.store.getAll('quarterly').map((reminder) => ({ ...reminder, type: 'quarterly', isAutomated: false }));
    const autoQuarterly = this.getQuarterlyReminders();
    return [...weekly, ...monthly, ...userQuarterly, ...autoQuarterly];
  }
  ...
  getReminderMeta(reminder) {
    ...
    const cycleInfo = DateUtils.getQuarterlyCycleInfo(reminder.month);
    const isDone = this.store.isQuarterlyCycleDone(reminder.id, cycleInfo.cycleKey);
    let countdown = DateUtils.calculateCountdown(cycleInfo.nextDate);

    if (cycleInfo.isGraceWindow) {
      countdown = isDone
        ? { text: 'Done', isDue: false, days: 0 }
        : { text: 'Overdue', isDue: true, days: 0 };
    }

    return {
      nextDate: cycleInfo.nextDate,
      countdown,
      ...
      isDone,
      isGraceWindow: cycleInfo.isGraceWindow,
      isAlertActive: cycleInfo.isGraceWindow && !isDone
    };
  }
  ...
  renderDueToday() {
    ...
    container.innerHTML = dueReminders.map((reminder) => {
      ...
      const showQuarterlyDoneAction = reminder.type === 'quarterly' && reminder.isAutomated && meta.isGraceWindow;
      const actionHtml = showQuarterlyDoneAction
        ? `
            <button
              onclick="toggleQuarterlyReminderDone('${reminder.id}')"
              class="reminder-action reminder-action-complete"
              title="${meta.isDone ? 'Mark as not done' : 'Mark as done'}"
            >
              <i class="fa-solid ${meta.isDone ? 'fa-rotate-left' : 'fa-check'}"></i>
            </button>
          `
        : reminder.type === 'quarterly' && reminder.isAutomated
          ? ''
        : `
            <button
    ...
    container.innerHTML = reminders.map(reminder => {
      ...
      return `
        <article class="reminder-item reminder-item-monthly ${meta.countdown.isDue ? 'reminder-item-due' : ''}">
          ...
          <div class="reminder-item-date ${meta.countdown.isDue ? 'reminder-item-date-due' : ''}">${DateUtils.formatDate(meta.nextDate)}${meta.countdown.isDue ? ' <span class="reminder-please-do-today">— Please Do Today</span>' : ''}</div>
          <div class="reminder-item-title">${title}</div>
          ${description ? `<div class="reminder-item-description">${description}</div>` : ''}
          <div class="reminder-item-meta">
            <span class="reminder-chip ${meta.chipClass}">
  ...
  renderQuarterly() {
    this.applyQuarterlyAutomationUiState();
    ...
    const userReminders = this.store.getAll('quarterly').map(r => ({ ...r, type: 'quarterly', isAutomated: false }));
    const autoReminders = this.getQuarterlyReminders();
    const reminders = [...userReminders, ...autoReminders];
    ...
    container.innerHTML = reminders.map(reminder => {
      ...
      let actionsHtml = '';
      if (reminder.isAutomated) {
        actionsHtml = meta.isGraceWindow ? `
          <button
            onclick="toggleQuarterlyReminderDone('${reminder.id}')"
            class="reminder-action reminder-action-complete"
            title="${meta.isDone ? 'Mark as not done' : 'Mark as done'}"
  ...
  applyQuarterlyAutomationUiState() {
    const formButton = document.getElementById('showQuarterlyFormBtn');
    if (formButton) {
      formButton.classList.remove('hidden');
    }

    const emptyTitle = document.querySelector('#noQuarterlyReminders .reminder-empty-title');
    if (emptyTitle) {
      emptyTitle.textContent = 'No quarterly reminders yet';
    }

    const emptyDescription = document.querySelector('#noQuarterlyReminders .reminder-empty-description');
    if (emptyDescription) {
      emptyDescription.textContent = 'Add a quarterly reminder or the system auto-generates MCMC quarterly report reminders.';
    }
  }

  getDueReminders() {
    const allReminders = this.getAllRemindersForUi();
    return allReminders.filter(reminder => {
      const meta = this.getReminderMeta(reminder);
      if (reminder.type === 'quarterly' && meta.isDone) {
        return false;
      }
      return DateUtils.isToday(meta.nextDate);
    });
  }
  ...
  getAlertReminders() {
    ...
    return allReminders.filter((reminder) => {
      ...
      if (reminder.type === 'quarterly') {
        return !meta.isDone && (DateUtils.isToday(meta.nextDate) || meta.isAlertActive);
      }
      return DateUtils.isToday(meta.nextDate);
    });
  }
  ...
  updateMarquee() {
    ...
    if (!marquee || !content || !windowEl) return;

    const dueReminders = this.getAlertReminders();

    if (dueReminders.length === 0) {
      marquee.classList.add('hidden');
      if (this._marqueeResizeObserver) {
        this._marqueeResizeObserver.disconnect();
    ...
    const buildMarquee = () => {
      ...
      const items = dueReminders.map(reminder =>
        `<span class="reminder-marquee-item">${escapeHtml(reminder.title)}</span>`
      );
      const sep = `<span class="reminder-marquee-sep" style="margin:0 0.75rem;">•</span>`;
      const baseRow = items.join(sep);

      // Measure a single copy width
      content.style.animation = 'none';
      content.innerHTML = `<span class="reminder-marquee-track-inner">${baseRow}</span>`;
      const singleWidth = content.querySelector('.reminder-marquee-track-inner').getBoundingClientRect().width;

      // For single item: pad to full windowWidth so copy 2 enters exactly when copy 1 exits
      const padWidth = dueReminders.length === 1
        ? Math.max(windowWidth - singleWidth, 32)
        : Math.max(windowWidth * 0.25, 32);

      const pad = `<span style="display:inline-block;width:${padWidth}px;" aria-hidden="true"></span>`;
      const fullRow = `${baseRow}${pad}`;
      const trackWidth = singleWidth + padWidth;

      const duration = Math.max(trackWidth / speedPxPerSec, 6);

      content.innerHTML =
        `<span class="reminder-marquee-track-inner">${fullRow}</span>` +
        `<span class="reminder-marquee-track-inner" aria-hidden="true">${fullRow}</span>`;
    ...
    // Attach resize observer so it adapts when screen size changes
    if (!this._marqueeResizeObserver) {
      this._marqueeResizeObserver = new ResizeObserver(() => buildMarquee());
      this._marqueeResizeObserver.observe(windowEl);
    }
  }
  ...
  updateSummary() {
    ...
    if (quarterlyCount) quarterlyCount.textContent = this.getQuarterlyReminders().length;
  ...
  updateBadge() {
    ...
    dueReminders.forEach(reminder => {
      const reminderTime = this.getReminderMeta(reminder).nextDate.getTime();
...
class ReminderManager {
  ...
  async deleteReminder(type, id) {
    if (type === 'quarterly' && id === 'quarterly-auto-mcmc-report') {
      alert('The automated MCMC quarterly reminder cannot be deleted.');
      return;
    }
  ...
  toggleQuarterlyDone(id) {
    const reminder = this.ui.getQuarterlyReminders().find((item) => item.id === id);
    if (!reminder) return;

    const meta = this.ui.getReminderMeta(reminder);
    if (!meta.cycleKey) return;

    this.store.setQuarterlyCycleDone(id, meta.cycleKey, !meta.isDone);
    this.ui.renderAllSections();
    this.ui.updateMarquee();
    this.ui.updateBadge();
    this.ui.updateSummary();
...
function toggleQuarterlyReminderDone(id) {
  reminderManager.toggleQuarterlyDone(id);
}
