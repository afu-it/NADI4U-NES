...
let nadi4uListTransitionEnabled = false;
...
async function copyToClipboard(text, btn) {
  ...
  const originalClass = icon?.className || "fa-regular fa-copy text-xs";
  const originalTitle = btn?.title || "Copy title";
  const clipboardUtils = window.NadiClipboardUtils;
  ...
  try {
    ...
    if (result?.ok) {
      ...
      setTimeout(() => {
        ...
        if (btn) btn.title = originalTitle;
      }, 1500);
      return;
    }
    ...
    throw result?.error || new Error("Copy failed");
  } catch (err) {
    if (!clipboardUtils?.isClipboardPolicyBlockedError?.(err)) {
...
function filterNadi4uMonthDataForCurrentSite(eventMetaRows, scheduleRows) {
  const events = Array.isArray(eventMetaRows) ? eventMetaRows.slice() : [];
  const schedule = Array.isArray(scheduleRows) ? scheduleRows.slice() : [];
  const userSiteId = getUserNadi4uSiteId();
  ...
  const filteredEvents = events.filter((eventMeta) => {
    const eventSiteIds = getNadi4uSiteIdList(eventMeta?.site_id);
    return eventSiteIds.length === 0 || eventSiteIds.includes(userSiteId);
  });
...
function resetNadi4uSectionPages() {
  window[getTodayProgramListPageStateKey()] = 0;
  window[getMultiProgramListPageStateKey()] = 0;
  window[getProgramListPageStateKey()] = 0;
}

function getNadi4uEventPillarKey(eventItem) {
  return String(eventItem?.kpiCategory || eventItem?.category || "")
    .trim()
    .toLowerCase();
}
...
function getNadi4uPillarAvailabilityCounts(eventList) {
  ...
  eventList.forEach((eventItem) => {
    const key = getNadi4uEventPillarKey(eventItem);
...
function getSortedNadi4uEventListForSection(eventList, options = {}) {
  if (!Array.isArray(eventList)) return [];
...
function getSectionedNadi4uDisplayData(sourceEvents) {
  const scopedResult = getNadi4uScopedFilterResult(sourceEvents);
  const baseVisibleEvents = Array.isArray(scopedResult.filteredBySearch)
  ...
  pillarFilteredEvents.forEach((eventItem) => {
    if (isNadi4uMultiDayEvent(eventItem)) {
...
function updateProgramListHeader() {
  ...
  const dayBtn = document.getElementById("programListTodaySummaryBtn");
  const multiBtn = document.getElementById("programListMultiSummaryBtn");
  ...
  applySummaryButtonStyles(dayBtn, dayCount > 0);
  applySummaryButtonStyles(multiBtn, multiCount > 0);

  NADI4U_PILLAR_BUTTON_CONFIG.forEach((config) => {
    ...
    if (!button.dataset.baseLabel) {
      button.dataset.baseLabel = button.textContent.split(" (")[0];
    }

    if (count <= 0) {
      button.classList.add(...config.disabled.split(" "));
      button.disabled = true;
...
window.scrollToProgramListSection = scrollToProgramListSection;

function handleNadi4uHeroBackgroundClick(event) {
  if (!nadi4uPillarFilter) return;
...
function setNadi4uListType(nextType) {
  ...
  nadi4uListTransitionEnabled = true;
  resetNadi4uSectionPages();
  renderEventList();
}
...
window.setNadi4uPillarFilter = setNadi4uPillarFilter;
...
function getBookmarkSectionsForMenu() {
  ...
  return sortSectionsForTopPriority(siteSettings.sections)
    .filter((section) => section && section.type !== "divider")
    .map((section) => {
...
function renderBookmarksMenu() {
  const container = document.getElementById("bookmarksDropdownContent");
  if (!container) return;

  container.innerHTML = "";
  const sections = getBookmarkSectionsForMenu();
  ...
  if (sections.length === 0) {
    ...
    text.textContent = "Add links in Manage Custom Links to see them here.";
  ...
  sections.forEach((section) => {
    ...
    const cols = Math.min(Math.max(section.cols, 1), 4);
    linksWrap.style.gridTemplateColumns = `repeat(${cols}, minmax(0, 1fr))`;
    if (cols >= 3) {
      linksWrap.style.minWidth = `${cols * 170}px`;
    }

    for (let columnIndex = 0; columnIndex < cols; columnIndex++) {
      const columnEl = document.createElement("div");
      columnEl.className = "flex min-w-0 flex-col gap-2";
      if (columnIndex > 0) {
      ...
      if (columnButtons.length === 0) {
        ...
        columnButtons.forEach((button) => {
          ...
          link.className =
            "flex w-fit max-w-full self-center items-center justify-between gap-2 rounded-lg border border-slate-200 bg-white px-2.5 py-2 text-[11px] font-semibold text-slate-700 transition-colors hover:border-nadi hover:text-nadi";
      ...
      linksWrap.appendChild(columnEl);
    }

    sectionEl.appendChild(linksWrap);
    container.appendChild(sectionEl);
  });
}

const BOOKMARKS_MENU_ANIMATION_MS = 300;

function showBookmarksMenu() {
  const dropdown = document.getElementById("bookmarksDropdown");
  const panel = dropdown?.querySelector(".bookmarks-panel");
  const backdrop = dropdown?.querySelector(".bookmarks-backdrop");
  if (!dropdown) return;

  if (dropdown._bookmarksCloseTimeout) {
    clearTimeout(dropdown._bookmarksCloseTimeout);
    dropdown._bookmarksCloseTimeout = null;
  }
...
function closeBookmarksMenu() {
  ...
  if (dropdown) {
    ...
    dropdown._bookmarksCloseTimeout = setTimeout(() => {
      dropdown.classList.add("hidden");
      dropdown._bookmarksCloseTimeout = null;
    }, BOOKMARKS_MENU_ANIMATION_MS);
  }

  const button = document.getElementById("bookmarksBtn");
  if (button) {
    button.setAttribute("aria-expanded", "false");
  }
}

function toggleBookmarksMenu(event) {
  ...
  if (willOpen && typeof window.closeHeaderLoginMenu === "function") {
...
document.addEventListener("click", (e) => {
  const dropdown = document.getElementById("bookmarksDropdown");
  const button = document.getElementById("bookmarksBtn");
  if (!dropdown || !button || dropdown.classList.contains("hidden")) return;
...
function renderCalendar() {
  ...
  if (!grid) return; // Calendar section hidden
...
function createProgramEventCardElement(ev) {
  ...
  if (ev?.isExternal && ev?.source === "nadi4u") {
    ...
    if (range.endDate) {
      ...
      if (!Number.isNaN(parsedEnd.getTime())) dateEndForDisplay = parsedEnd;
    }
  }
      ...
      <div class="flex justify-between items-start gap-2">
        <div class="flex-1 min-w-0">
          <div class="flex items-baseline gap-2">
            ${displaySubcategory ? `<span class="subcategory text-[9px] font-semibold ${cat.color.split(" ")[1]}">${displaySubcategory}</span>` : ""}
            ${programTypeLabel ? `<span class="program-type text-[10px] font-bold tracking-wide" style="color:#6b7280;">- ${escapeHtml(translatedProgramType)} -</span>` : ""}
          </div>
          <div class="program-title-wrapper mt-0.5">
            <span class="program-title text-sm font-bold text-slate-800 leading-tight break-words">${ev.title}</span>
            <button onclick="copyToClipboard('${escapeHtml(ev.title.replace(/'/g, "\\'"))}', this)" class="text-slate-300/60 hover:text-slate-500 focus-visible:text-slate-500 cursor-pointer inline-flex align-baseline ml-2 transition-colors duration-150" title="Copy title">
              <i class="fa-regular fa-copy text-xs"></i>
            </button>
          </div>
          <div class="flex items-baseline gap-2 mt-1">
            <span class="date text-[10px] text-slate-500 font-medium"><i class="fa-regular fa-calendar mr-1"></i>${dateDisplayHtml}</span>
            ${ev.time || ev.secondTime ? `<span class="time text-[10px] text-slate-500 font-medium"><i class="fa-regular fa-clock mr-1"></i>${ev.time || ""}${ev.secondTime ? ` - ${ev.secondTime}` : ""}</span>` : ""}
          </div>
        </div>
...
function getProgramListEventsPerPage(sectionKey = "today") {
  return 10;
}

function getSectionProgramListPageStateKey(sectionKey) {
  return sectionKey === "multi"
...
function renderProgramEventSection(rootContainer, options = {}) {
  const sectionKey = options?.sectionKey === "multi" ? "multi" : "today";
  ...
  let currentPage = window[pageStateKey];
  ...
  if (eventList.length === 0) {
    ...
    listContainer.innerHTML = `
      <div class="text-center p-5 bg-slate-50 rounded-lg border border-dashed border-slate-300 text-slate-500 text-xs">
        <i class="fa-regular fa-calendar-xmark text-2xl text-slate-300 mb-2"></i>
        <p class="font-medium">${escapeHtml(emptyTitle)}</p>
        ${emptySubtitle ? `<p class="text-[10px] text-slate-400 mt-1">${escapeHtml(emptySubtitle)}</p>` : ""}
      </div>
    `;
  } else {
    paginatedEvents.forEach((eventItem) => {
      const card = createProgramEventCardElement(eventItem);
  ...
  rootContainer.appendChild(sectionEl);
...
function renderEventList() {
  ...
  container.classList.remove("program-list-ready");
  if (nadi4uListTransitionEnabled) {
    container.classList.add("program-list-animating");
  } else {
    container.classList.remove("program-list-animating");
  }
  container.innerHTML = "";
  eventImageMap = {};
  ...
  if (!isNadi4uView && displayEvents.length === 0) {
    container.innerHTML = `
  ...
  let visibleTodayEvents = [];
  let visibleMultiEvents = [];
  if (nadi4uListType === NADI4U_LIST_TYPE_MULTI) {
  ...
  const visibleEventIds = [...visibleTodayEvents, ...visibleMultiEvents]
  ...
  if (nadi4uListTransitionEnabled) {
    requestAnimationFrame(() => {
      container.classList.add("program-list-ready");
      container.classList.remove("program-list-animating");
      nadi4uListTransitionEnabled = false;
    });
  } else {
    container.classList.add("program-list-ready");
  }
...
window.changeSectionEventPage = changeSectionEventPage;
window.setSectionEventPage = setSectionEventPage;
...
async function syncNADI4UData(options = {}) {
  ...
  if (window.NADI4U_API) {
    ...
    try {
      ...
      document.getElementById("nadi4uPreviewContent").textContent =
...
// Track whether the on-load sync has been triggered so the window.load
// fallback (needed for Google Sites iframes) does not double-fire it.
let _nadi4uOnLoadSyncTriggered = false;

async function _initAndSyncOnLoad() {
  if (_nadi4uOnLoadSyncTriggered) return;
  _nadi4uOnLoadSyncTriggered = true;
...
let calendarMiniBtnRef = null;
let calendarMiniResizing = false;

function positionCalendarMini() {
...
function toggleCalendarMini(event) {
  ...
  if (!dropdown || !wrap || !btn) return;

  calendarMiniBtnRef = btn;
...
function closeCalendarMiniOnDocClick(e) {
  const dropdown = document.getElementById("calendarMiniDropdown");
  if (!dropdown) return;
  // Use composedPath() which is captured at dispatch time, before any DOM
  // mutation (e.g. renderMiniCalendar replacing innerHTML removes the clicked
  // button from the tree, making contains() return false incorrectly).
