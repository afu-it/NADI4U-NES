...
const NADI4U_API = {
  ...
  async getProgrammesNadi4uMonthEventIds(year, month) {
    this.ensureAuth();

    const monthStart = new Date(year, month, 1).toISOString();
    const monthEnd = new Date(year, month + 1, 0, 23, 59, 59, 999).toISOString();
    const url = `${this.baseUrl}/nd_event?select=id&category_id=eq.1&status_id=neq.1&start_datetime=lte.${encodeURIComponent(monthEnd)}&end_datetime=gte.${encodeURIComponent(monthStart)}`;
  ...
  async getSmartServicesNadi4uMonthData(year, month) {
    ...
    const eligibleProgrammeIds = await this.getProgrammesNadi4uMonthEventIds(year, month);
    const takwimEvents = Array.isArray(eventsRaw)
      ? eventsRaw.filter((eventItem) => this.getTakwimCategoryGroup(eventItem?.nd_event_category?.name) === 'nadi4u')
      : [];
    const events = takwimEvents.filter((eventItem) => eligibleProgrammeIds.has(eventItem?.id));

    if (window.DEBUG_MODE && takwimEvents.length !== events.length) {
      const droppedEventTitles = takwimEvents
        .filter((eventItem) => !eligibleProgrammeIds.has(eventItem?.id))
        .map((eventItem) => eventItem?.program_name)
        .filter(Boolean);
      console.info('NADI4U takwim events excluded because they are not in /programmes/nadi4u:', droppedEventTitles);
    }
